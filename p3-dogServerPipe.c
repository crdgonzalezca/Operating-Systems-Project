...
void historia_control(int registro, int socket_desc){
	Dog dog_ver;
	int r;
	FILE *historia;
	char path[35];
...
void ver(int socket_desc){
	...
	int pos = find_visited(registro);
	if(pos == -1){
		visited[pos] = registro;
		read(pipe_server[0], &testigo, 1);
		historia_control(registro, socket_desc);
		write(pipe_server[1], "I", 1);
		visited[pos] = -1;
	} else {
		while(visited[pos] != -1);
...
uno por uno.*/
void buscar(int socket_desc){
	/*Se debe configurar la funcion para que reciba el nombre a buscar y se le envien al
	cliente, estructura por estructura, los perros que coincidan.*/
